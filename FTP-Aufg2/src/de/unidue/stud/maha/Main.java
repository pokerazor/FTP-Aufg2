...
public class Main extends SoFT {
	...
	public static void main(String[] args) {
		Node[] nodes = { new Prozess("A"), new Prozess("B"), new Prozess("C"),
				new Maskierer("D"), new Maskierer("E"), new Maskierer("F") };
	...
	public int result(String input, String[] output) {
		int resultVal = determineResult(number(words(input, 1, 1, 1)),
				number(words(input, 2, 1, 1)), number(words(input, 3, 1, 1)),
				number(words(output[3], 1, 1, 1)),
				number(words(output[4], 1, 1, 1)),
				number(words(output[5], 1, 1, 1)));
		return resultVal;
	}
	...
	public int determineResult(int firstInput, int secondInput, int thirdInput,
		...
		if (isValid(firstValue, min, max) && isValid(secondValue, min, max)
				&& isValid(thirdValue, min, max)) {
